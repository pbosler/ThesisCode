...
if ( procRank == 0 ) then
	...
	elseif (panelKind == QUAD_PANEL ) then
		if ( AMR == 0) then
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_DIRECT_quadNest',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_DIRECT_quadNest',initNest,'_rev',tfinal,'_dt',dt,'_'
		else
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_DIRECT_quadAMR',initNest,'_rev',tfinal,'_dt',dt,'.dat'
