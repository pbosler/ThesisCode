...
integer(kint), parameter :: problemID = BLOCKM
...
logical(klog) :: newEst
integer(kint), parameter :: INTEGER_INIT_SIZE = 6, &
				 			REAL_INIT_SIZE = 4
integer(kint) :: procRank, numProcs, mpiErrCode, intBuffer(INTEGER_INIT_SIZE)
real(kreal) :: realBuffer(REAL_INIT_SIZE)
...
if ( procRank == 0 ) then
	...
	open (unit=readUnit,file='AdvectBlockM.namelist',action='READ',status='OLD',iostat=readStat)
		if ( readStat /= 0) then
	...
	if ( panelKind == TRI_PANEL ) then
		if ( AMR == 0) then
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_LG_triNest',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_triNest',initNest,'_rev',tfinal,'_dt',dt,'_'
		else
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_LG_triAMR',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_triAMR',initNest,'_rev',tfinal,'_dt',dt,'_'
		endif
	elseif (panelKind == QUAD_PANEL ) then
		if ( AMR == 0) then
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_LG_quadNest',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_quadNest',initNest,'_rev',tfinal,'_dt',dt,'_'
		else
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'_LG_quadAMR',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_quadAMR',initNest,'_rev',tfinal,'_dt',dt,'_'
		endif
	else
...
call MPI_BCAST(intBuffer,INTEGER_INIT_SIZE,MPI_INTEGER,0,MPI_COMM_WORLD,mpiErrCode)
...
call InitBlockM(gridParticles,gridPanels)
...
do j=1,gridPanels%N
	if ( .NOT. gridPanels%hasChildren(j) ) then
		gridPanels%tracer(j,2) = abs( BlockMX(gridPanels%x(:,j)) - gridPanels%tracer(j,1))
	endif
enddo
