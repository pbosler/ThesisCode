...
if ( procRank == 0) then
	...
	open(unit=readUnit,file='RH2Panels.namelist',action='READ',status='OLD',iostat=readStat)
		if (readstat /= 0 ) then
...
call InitRH2Wave(gridParticles,gridPanels)
if ( AMR > 0 ) then
