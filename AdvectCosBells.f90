...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
	...
	if ( procRank == 0) then
		...
		if ( panelKind == TRI_PANEL ) then
			if ( AMR == 0) then
				write(dataFile,'(A,A,I1,A,F3.1,A,F5.3)') trim(jobPrefix),'_LG_triNest',initNest,'_rev',tfinal,'_dt',dt
				write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_triNest',initNest,'_rev',tfinal,'_dt',dt,'_'
			else
				write(dataFile,'(A,A,I1,A,F3.1,A,F5.3)') trim(jobPrefix),'_LG_triAMR',initNest,'_rev',tfinal,'_dt',dt
		...
		elseif (panelKind == QUAD_PANEL ) then
			if ( AMR == 0) then
				write(dataFile,'(A,A,I1,A,F3.1,A,F5.3)') trim(jobPrefix),'_LG_quadNest',initNest,'_rev',tfinal,'_dt',dt
				write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'_LG_quadNest',initNest,'_rev',tfinal,'_dt',dt,'_'
			else
				write(dataFile,'(A,A,I1,A,F3.1,A,F5.3)') trim(jobPrefix),'_LG_quadAMR',initNest,'_rev',tfinal,'_dt',dt
