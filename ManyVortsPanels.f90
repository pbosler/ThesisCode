...
logical(klog) :: remeshFlag , newEst
integer(kint), parameter :: problemID = MULTIPLE_VORTICES
...
if ( procRank == 0) then
	...
	open(unit=readUnit,file='ManyVortsPanels.namelist',action='READ',status='OLD',iostat=readStat)
		if (readstat /= 0 ) then
...
call InitManyVorts(gridParticles,gridPanels)
if ( AMR > 0 ) then
...
do timeJ = 0,timesteps-1
	...
	if ( (procRank == 0 .AND. timeJ == 1) .OR. (procRank == 0 .AND. newEst )) then
