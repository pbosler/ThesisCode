module VoronoiVelocityParallelModule
...
integer(kint), allocatable :: panelIndexStart(:), panelIndexEnd(:), panelMessageSize(:)
...
character(len=28) :: logKey = 'VoronoiVelocityMPI : '
type(Logger) :: log
...
subroutine InitRK4(aPanels,numProcs)
	type(VorPanels), intent(in) :: aPanels
	integer(kint), intent(in) :: numProcs
	call LoadBalance(aPanels%N,numProcs)
...
subroutine InitializeMPIRK4(nPanels,procRank,numProcs)
	integer(kint), intent(in) :: nPanels
	integer(kint), intent(in) :: procRank
	integer(kint), intent(in) :: numProcs
	...
	allocate(panelIndexStart(0:numProcs-1))
	allocate(panelIndexEnd(0:numProcs-1))
	allocate(panelMessageSize(0:numProcs-1))
	panelIndexStart = 0
	panelIndexEnd = 0
	panelMessageSize = 0
	mpiIsReady = .True.
	call LoadBalance(nPanels,numProcs)
end subroutine
...
subroutine FinalizeMPIRK4()
	call ResetRK4()
	deallocate(panelIndexStart)
	deallocate(panelIndexEnd)
	deallocate(panelMessageSize)
	mpiIsReady = .False.
end subroutine


subroutine LoadBalance(n,p)
	integer(kint), intent(in) :: n,p
	integer(kint) :: chunkSize, j
	if ( mpiIsReady ) then
		chunkSize = n/p
		do j=0,p-1
			panelIndexStart(j) = j*chunkSize + 1
			panelIndexEnd(j) = (j+1)*chunkSize
		enddo
		panelIndexEnd(p-1) = n
		panelMessageSize = panelIndexEnd - panelIndexStart + 1
	else 
		call LogMessage(log,ERROR_LOGGING_LEVEL,logKey,'LoadBalance ERROR : MPI arrays not allocated.')
		return
	endif
end subroutine
...
subroutine BVERK4(aPanels, dt, procRank, numProcs)
	type(VorPanels), intent(inout) :: aPanels
	real(kreal), intent(in) :: dt
	integer(kint) :: procRank, numProcs, j
	real(kreal) :: norm
	
	!call LogMessage(log,DEBUG_LOGGING_LEVEL,logkey,'Entering BVERK4.')
	if ( .NOT. mpiIsReady ) then
		call LogMessage(log,ERROR_LOGGING_LEVEL,logKey,'RK4 ERROR : MPI not ready.')
		return
	endif
	
	if ( .NOT. rk4isReady ) then
		call InitRK4(aPanels,numProcs)
	else
		call ZeroRK4()	
	endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!		
	...
	call BVEVelocity( stage1X,stage1Y,stage1Z,stage1Vort, & !output data 
					  ...
					  panelIndexStart(procRank), panelIndexEnd(procRank)) ! indices
	! broadcast results to all
	do j=0,numProcs-1
		call MPI_BCAST(stage1X(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage1Y(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage1Z(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage1Vort(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)			   			   			   
	enddo				  
	...
	call BVEVelocity( 	stage2x, stage2y, stage2z, stage2vort, &
						...
						panelindexStart(procRank), panelindexEnd(procRank))
	do j=0,numProcs-1
		call MPI_BCAST(stage2X(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage2Y(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage2Z(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage2Vort(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)			   			   			   
	enddo						
	stage2x = dt*stage2x
	stage2y = dt*stage2y
	...
	call BVEVelocity( 	stage3x, stage3y, stage3z, stage3vort, &
						...
						panelindexStart(procRank), panelindexEnd(procRank))
	do j=0,numProcs-1
		call MPI_BCAST(stage3X(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage3Y(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage3Z(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage3Vort(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)			   			   			   
	enddo						
	...
	call BVEVelocity( 	stage4x, stage4y, stage4z, stage4vort, &
						...
						panelindexStart(procRank), panelindexEnd(procRank))
	do j=0,numProcs-1
		call MPI_BCAST(stage4X(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage4Y(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage4Z(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)
		call MPI_BCAST(stage4Vort(panelIndexStart(j):panelIndexEnd(j)), &
					   panelMessageSize(j),MPI_DOUBLE_PRECISION,j,MPI_COMM_WORLD,mpiErrorCode)			   			   			   
	enddo						
	...
	call NormalizeGrid(aPanels)! Renormalize for STRIPACK
