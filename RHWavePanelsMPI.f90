...
real(kreal) :: amrTol1, amrTol2, circTol, varTol, lagVar, baseVar, minX0(3), maxX0(3), maxCirc
logical(klog) :: useRelativeTol
...
if ( procRank == 0) then
	...
	if ( panelKind == TRI_PANEL ) then
		if ( AMR == 0) then
			write(dataFile,'(A,A,A,I1,A,F3.1,A,F5.3,A)') trim(outputDir),trim(jobPrefix),'LG_triNest',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,A,I1,A,F3.1,A,F5.3,A)') trim(outputDir),'vtkOut/',trim(jobPrefix),'LG_triNest',initNest,'_rev',tfinal,'_dt',dt,'_'
			write(vtkRoot2,'(A,A)') trim(vtkRoot),'_active_'
		else
			write(dataFile,'(A,A,I1,A,F3.1,A,F5.3,A)') trim(jobPrefix),'LG_triAMR',initNest,'_rev',tfinal,'_dt',dt,'.dat'
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'LG_triAMR',initNest,'_rev',tfinal,'_dt',dt,'_'
			write(vtkRoot2,'(A,A)') trim(vtkRoot),'_active_'
		endif
	elseif (panelKind == QUAD_PANEL ) then
		if ( AMR == 0) then
			...
			write(vtkRoot,'(A,A,A,I1,A,F3.1,A,F5.3,A)') 'vtkOut/',trim(jobPrefix),'LG_quadAMR',initNest,'_rev',tfinal,'_dt',dt,'_'
			write(vtkRoot2,'(A,A)') trim(vtkRoot),'_active_'
		endif
	else
	...
	write(vtkFile2,'(A,I0.4,A)')trim(vtkRoot2),0,'.vtk'
endif
...
do timeJ = 0,timesteps-1
	...
	if ( procRank == 0 .AND. timeJ == 1) then
		...
		write(logString,'(A,F9.3,A)') 'Estimated time left = ',etime*real(timesteps,kreal)/(60.0*real(timeJ,kreal)),' minutes.'
	...
	if ( procRank == 0 ) then
		...
		write(vtkFile2,'(A,I0.4,A)')trim(vtkRoot2),timeJ,'.vtk'
		call vtkOutput(gridParticles,gridPanels,vtkFile)
	endif
enddo
