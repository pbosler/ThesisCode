...
/* CHANGE DATA PARAMETERS HERE */
	double dt = 0.01;
	double relVortMin = -12.5;
	double relVortMax = 12.5;
	int remeshInterval = 40;
	bool rotation = false;
...
/* TO DO : READ DATA PARAMETERS AT RUN TIME */	
	...
	if ( perturbation ){
		strcpy(titleString1,"Perturbed Jet ");
	}
	else{
		strcpy(titleString1,"Steady Jet ");
		if ( rotation )
			strcat(titleString1," (with rotation)");
		else
			strcat(titleString1," (no rotation)");	
	}
...
// Setup vtk pipeline for left panel (whole sphere)
...
// Setup vtk pipeline for right panel (zoom in)	
	...
	vtkPolyDataMapper *vorMapper2=vtkPolyDataMapper::New();
		//delMapper2->SetScalarRange(relVortMin,relVortMax);
		vorMapper2->SetScalarRange(relVortMin,relVortMax);
		vorMapper2->SetLookupTable(vortScale);
		//delMapper2->SetLookupTable(vortScale);
	...
	vtkCamera *camera2=vtkCamera::New();
		//camera2->SetPosition(1.2,-0.6,1.1);
		camera2->SetPosition(1.44,-0.72,1.32);
...
// Set up output filenames and writer
	...
	strcpy(jpgFileName,"jpgOut/jetVoronoi2panel_");
...
// Rendering loop
	...
	for (int frameJ=0; frameJ<=finalFrame; frameJ++){	
		...
		delMapper2->SetInput(DelaunayData[frameJ]->GetOutput());
		vorMapper1->SetInput(VoronoiData[frameJ]->GetOutput());
		vorMapper2->SetInput(VoronoiData[frameJ]->GetOutput());
	...
	delMapper2->SetInput(DelaunayData[finalFrame]->GetOutput());
	vorMapper1->SetInput(VoronoiData[finalFrame]->GetOutput());
	vorMapper2->SetInput(VoronoiData[finalFrame]->GetOutput());
