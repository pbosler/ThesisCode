...
integer(kint) :: problemID=GAUSSIAN_HILLS
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
	...
	if ( procRank == 0) then
		...
		open(unit=readUnit,file='AdvectGaussHills.namelist',action='READ',status='OLD',iostat=readStat)
			if (readstat /= 0 ) then
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
	...
	call InitGaussianHills(gridParticles,gridPanels)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
	...
	do j=1,gridPanels%N
		if ( .NOT. gridPanels%hasChildren(j) ) then
			gridPanels%tracer(j,2) = abs( GaussianHillsX(gridPanels%x(:,j)) - gridPanels%tracer(j,1))
		endif
	enddo
