...
private
public TRMESH, CRLIST
public TRLIST2

type(Logger) :: log
integer(kind=4), parameter :: logLevel = WARNING_LOGGING_LEVEL
character(len=28)  :: logKey = 'STRIPACK : '
logical,  save :: logInit = .False.
...
!
!type STRIPACKData
!	real(kreal), dimension(:), pointer :: x, y, z, xc, yc, zc, rc
!	real(kreal), dimension(:), pointer :: dist
!	integer(kint), dimension(:), pointer :: list, lptr, lend, listc
!	integer(kint), dimension(:), pointer :: near, next
!	integer(kint), dimension(:,:), pointer :: ltri
!	integer(kint) :: lnew, nb, ncol	
!end type
!
!interface New
...
subroutine InitLogger(aLog)
	...
	integer(kind=4) :: logUnit
	logUnit = 6
	call New(aLog,logLevel,logUnit)
	logInit = .TRUE.
end subroutine

!subroutine NewPrivate(self,nGens)
!	type(STRIPACKData), intent(out) :: self
!	integer(kind=4), intent(in) :: nGens
!	integer(kind=4) :: nTri, nArcs
!	
!	nTri = 2*nGens - 4
!	nArcs = 3*nGens - 6
!	
...
function det (x1,y1,z1,x2,y2,z2,x0,y0,z0) 
	real(kind=8), intent(in) :: x1,y1,z1, x2,y2,z2, x0,y0,z0
	real(kind=8) :: det
...
!  call LogMessage(log,DEBUG_LOGGING_LEVEL,logKey,'Entering TRFIND.')
...
!
  ...
  call LogMessage(log,DEBUG_LOGGING_LEVEL,logKey,'Entering TRLIST2.')
...
!
 ...
 ! logical              left
  ...
  call LogMessage(log,DEBUG_LOGGING_LEVEL,logKey,'Entering TRMESH.')
