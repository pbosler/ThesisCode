...
integer(kint), parameter :: problemKind = BVE_SOLVER, problemID = TWO_VORTS
logical(klog) :: remeshFlag
real(kreal) :: amrTol1, circTol, amrTol2, baseVar, varTol, minX0(3), maxX0(3), maxCirc

! vorticity variables
real(kreal) :: amp1, beta1, lat1, lon1, amp2, beta2, lat2, lon2
real(kreal) :: cent1(3), cent2(3)
...
integer(kint), parameter :: REAL_INIT_BUFFER_SIZE = 12, INTEGER_INIT_BUFFER_SIZE = 4
...
namelist /vortInit/ amp1, beta1, lat1, lon1, amp2, beta2, lat2, lon2
...
if (procRank == 0) then
	...
	open(unit=readUnit,file='TwoVorts.namelist',action='READ',status='OLD',iostat=readStat)
		if (readStat /= 0) then
...
cent1 = [ cos(lon1)*cos(lat1), sin(lon1)*cos(lat1), sin(lat1) ]
cent2 = [ cos(lon2)*cos(lat2), sin(lon2)*cos(lat2), sin(lat2) ]
...
call InitTwoVorts(gridParticles,gridPanels, cent1, cent2, beta1, beta2, amp1, amp2 )
if (AMR>0) then
	...
	call InitRefine(gridParticles,gridEdges,gridPanels,circTol,varTol,problemID,procRank=procRank,&
					cent1=cent1,cent2=cent2,beta1=beta1,beta2=beta2,strength1=amp1,strength2=amp2)
	call InitTwoVorts(gridParticles,gridPanels,cent1, cent2, beta1, beta2, amp1, amp2 )
else
...
if ( procRank == 0 ) then
	...
	if ( panelKind == 3) then
		write(dataFile,'(A,A,A,A,F4.2,A,F6.4,A)') trim(jobPrefix),'_tri',trim(amrString),'_rev',tfinal,'_dt',dt,'_'
	elseif (panelKind == 4) then
		write(dataFile,'(A,A,A,A,F4.2,A,F6.4,A)') trim(jobPrefix),'_quad',trim(amrString),'_rev',tfinal,'_dt',dt,'_'
	endif
	write(vtkRoot,'(A,A)') 'vtkOut/',trim(dataFile)
