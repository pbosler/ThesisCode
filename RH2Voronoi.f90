...
integer(kint), parameter :: problemID = RH2_WAVE
...
if ( procRank == 0 ) then
	...
	open(unit=readUnit,file='RH2WaveVoronoi.namelist',action='READ',status='OLD',iostat=readStat)
		if ( readStat /= 0) then
...
do timeJ=0,timesteps-1
	...
	do j=1,panels%N
		panels%tracer(j,3) = abs(panels%relVort(j) - &
				RH2WaveX(panels%x(j),panels%y(j),panels%z(j)))
	enddo
